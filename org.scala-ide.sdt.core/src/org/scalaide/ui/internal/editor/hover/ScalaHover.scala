    ...
    icu.withSourceFile({ (src, compiler) =>
      ...
      val docComment = {
        val thisComment = {
          ...
          for ((sym, site, header) <- askedOpt) yield parsedDocComment(sym, site, icu.scalaProject.javaProject).map { (comment) => (new ScalaDocHtmlProducer).getBrowserInput(compiler, icu.scalaProject.javaProject)(comment, sym, header.getOrElse(""))
          }
        }
        thisComment.flatten
      }
