  ...
  override def getHoverInfo2(viewer: ITextViewer, region: IRegion): AnyRef = {
    val docComment = icu.withSourceFile({ (src, compiler) =>
      ...
      val thisComment = scompiler flatMap { (compiler) =>
        ...
        val tree = askTypeAt(wordPos).getOption()
        ...
        for ((sym, site, header) <- askedOpt) yield parsedDocComment(sym, site).map { (comment) => HtmlProducer(comment, sym, header.getOrElse("")) }
      }
      thisComment.flatten
    }).flatten
